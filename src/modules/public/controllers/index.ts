    ...
    static findElectionsActual = async (req: Request, res: Response, next: NextFunction) => {
        try {
            const Elections = await electionRepository.findOne({
                ...
                order: { id: -1, statusSchedule: { status: { id: 1 } }, ballots: { index: +1, ballotItems: { index: +1, ballotItemSubjects: {index: +1}, ballotItemValues: {votedValue: -1, index: +1} } } }
            });
    ...
    static findElectionsDetail = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const Elections = await electionRepository.findOne({
                ...
                order: { statusSchedule: { status: { id: -1 } }, ballots: { index: +1, ballotItems: { index: +1, ballotItemSubjects: {index: +1} } } }
            });
            return res.json(Elections);
        } catch (error) {
            next(error)
