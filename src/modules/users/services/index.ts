...
export const addOTP = async (target: string, deviceUid: string, type: string, value: string, createdBy: number) => {
    ...
    const result = { id: otp.id, code: otp.code, type: otp.type, value: otp.value, status: "otp_send_successfuly" }
    return result
}
...
export const checkUserInivitation = async (mobileNumber: string, personalId: string) => {
